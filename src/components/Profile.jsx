...
const Profile = () => {
    ...
    const handleUpdateUser = async (id) => {
        ...
        try {
            const response = await axios.put(`http://localhost:8000/api/update/${id}`, { name: newName, email: newEmail }); // Use template literals for URL
    ...
    const handleDeleteUser = async (id) => {
        try {
            const response = await axios.delete(`http://localhost:8000/api/delete/${id}`); // Use template literals for URL
            if (response.status === 200) {
